...
class WasdInterface(object):
    ...
    def _drive_cmd(self, key):
        ...
        try:
            ...
            if key and key != -1 and key < 256:
                LOGGER.warning(f'Unrecognized keyboard command: \'{chr(key)}\'')

    def _try_grpc(self, desc, thunk):
        try:
            return thunk()
        except (ResponseError, RpcError, LeaseBaseError) as err:
            LOGGER.error(f'Failed {desc}: {err}')
            return None
    ...
    def _move_forward(self, time_secs=VELOCITY_CMD_DURATION):
        self._velocity_cmd_helper('move_forward', v_x=VELOCITY_BASE_SPEED, time_duration=2)

    def _move_backward(self):
        self._velocity_cmd_helper('move_backward', v_x=-VELOCITY_BASE_SPEED, time_duration=2)
    ...
    def _velocity_cmd_helper(self, desc='', v_x=0.0, v_y=0.0, v_rot=0.0, time_duration=VELOCITY_CMD_DURATION):
        self._start_robot_command(
            desc, RobotCommandBuilder.synchro_velocity_command(v_x=v_x, v_y=v_y, v_rot=v_rot),
            end_time_secs=time.time() + time_duration)
    ...
    def _toggle_power(self):
        ...
        if power_state is None:
            LOGGER.error('Could not toggle power because power state is unknown')
            return
...
def _setup_logging(verbose):
    ...
    LOGGER.propagate = False  # keep logs local to this logger
...
# python
def runRealAutowalk(wasd_interface):
    """Execute autonomous mission with state checks."""
    # Release software E‑Stop and wait until NOT_ESTOPPED
    LOGGER.info("Releasing E-Stop...")
    wasd_interface._toggle_estop()  # starts EstopKeepAlive

    LOGGER.info(wasd_interface.estop_str)

    # Power on
    LOGGER.info("Powering on robot...")
    wasd_interface._toggle_power()

    LOGGER.info(wasd_interface._power_state_str)
    ...
    LOGGER.info("Autowalk completed!")
